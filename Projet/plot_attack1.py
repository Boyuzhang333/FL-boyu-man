#!/usr/bin/env python3
"""
Attack1 (Label Flipping) Results Visualization | 攻击1（标签翻转）结果可视化
...
def collect_results():
    ...
    # Traverse all possible files | 遍历所有可能的文件
    for data_split in ['iid', 'non_iid_class']:
        for n_mal in [0, 1, 2, 3]:
            for run_id in range(5):  # 5 repetitions | 5次重复
...
def plot_results():
    ...
    # Set plot style | 设置图表样式
    plt.style.use('default')
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 5))
    
    malicious_clients = [0, 1, 2, 3]
    
    # Plot 1: IID case | 图1: IID情况
    iid_means = []
    iid_stds = []
    ...
    # Plot 2: Non-IID case | 图2: Non-IID情况
    non_iid_means = []
    non_iid_stds = []
    ...
    # Display statistical information | 显示统计信息
...
if __name__ == "__main__":
    # Check if results1 directory exists | 检查results1目录是否存在
    if not os.path.exists('results1'):
        print("Error: results1/ directory not found!")
        exit(1)
    
    # Check number of CSV files | 检查CSV文件数量
